...
public class videooption extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        b1.setOnClickListener(view -> {
            if (selectedVideoPath != null) {
                ...
                Toast.makeText(videooption.this, "Please select a video first", Toast.LENGTH_SHORT).show();
            }
    ...
    private void uploadVideo(final String selectedPath) {
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    String serverURL = ip.ipn+"videooption.php";
                    ...
                    if (serverResponseCode == 200) {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(videooption.this, "Video uploaded successfully", Toast.LENGTH_SHORT).show();
                            }
                        });
                    } else {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(videooption.this, "Failed to upload video", Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
